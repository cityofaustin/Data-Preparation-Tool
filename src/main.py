...
class trimWindow(QtWidgets.QMainWindow, Ui_trimWindow):
    ...
    def updateInterface(self):
        if trimInfo.isRight == True:
            trimSlider.setRange(-len(trimInfo.trimText), 0)
    ...
    def leftSelected(self, selected):
        if selected:
            trimLabel.setText(f"<span style='color: black'>{trimInfo.trimText}</span>")
...
def write_dt_to_qTable(df: pd.DataFrame, table: QtWidgets.QTableWidgetItem):
    ...
    for i, header in enumerate(headers):
        if len(header) > 10:
            headersShorten.append(str(header[:10] + "..."))
        else:
    ...
    for i, header in enumerate(headers):
        colDtype = str(df.iloc[:,i].dtypes)
        headersShorten[i] = f"{headersShorten[i]} ({colDtype})"
